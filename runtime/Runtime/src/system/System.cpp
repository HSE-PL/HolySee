...
auto sys::salloc(size_t size, size_t tier) -> void* {
  ...
  if (MemoryManager::regions_->at(tier).slots_.empty()) {
    if (MemoryManager::cur + size > Allocator::instance().heap_start + max_heap_size)
      return nullptr;

    start_new_object = MemoryManager::cur;
    MemoryManager::cur += MemoryManager::regions_->at(tier).t_size;
  } else {
    if (!MemoryManager::regions_->at(tier).slots_.try_pop(start_new_object))
      return nullptr;
  }
...
auto sys::sfree(Arena* a) -> void {
  MemoryManager::regions_->at(a->tier).slots_.push(reinterpret_cast<void*>(a->start));
